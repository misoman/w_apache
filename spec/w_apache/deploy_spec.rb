...
describe 'w_apache::deploy' do
	context 'with default setting' do
	  ...
	  it 'creates /root/.ssh and /root/.ssh/jenkins_pub and add and delete jenkins_pub' do
	    expect(chef_run).to create_directory('/root/.ssh').with(owner: 'root', group: 'root', mode: '0700')
	    expect(chef_run).to create_file('/root/.ssh/jenkins_pub').with(owner: 'root', group: 'root', content: 'ssh-rsa  jenkins@jenkins.examplewebsite.com')
	    expect(chef_run).to run_execute('add and delete jenkins_pub')
	  end
